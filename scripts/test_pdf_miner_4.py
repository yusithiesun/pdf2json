    ...
    for i in index_li:
        if len(i) > 0:
            if i[0] != '图' and i!=('' or ' ' or '\n') and i[0] != '表':
                if pattern.match(i) != None and pattern.match(i).group(0) not in title_set and len(re.findall('。|！|？|，|；|年|月|日|元|\%|0',i)) == 0:        
                    index_list.append(pattern.match(i).group(0))
                    l = len(pattern.match(i).group(0))
                    title_set.add(pattern.match(i).group(0))
                elif digit_pattern.match(i)!= None and digit_pattern.match(i).group(0) not in title_set and len(re.findall('。|！|？|，|；|年|月|日|元|\%|0',i)) == 0:        
                    ...
                    if len(re.findall('。|！|？|，|、|；',txt)) == 0:
                        ...
                        # 小标题
                            ...
                            if len(txt.replace('\n','').replace(' ','')) > 0 and len(re.findall('\.',txt)) < 4:
                                paragraphed_chapter['content'].append(txt.replace('/n','').replace(' ',''))
                            para = ''
                    else:
                        # 正文:有句子结束符
                        if len(re.findall('^[图|表|资料来源]',txt)) <= 0 and len(re.findall('\.',txt)) < 4 and len(txt) >= 3:
                            para += txt.replace('\n','').replace(' ','')
...
if __name__ == "__main__":
    ...
    src_f = open("/Users/yusi/codes/thu/data/data/eastmoney_report_data/"+src+".json",'r')
    target_f = open("/Users/yusi/codes/thu/data/data/processed-eastmoney_report_data/"+src+".json",'a')
